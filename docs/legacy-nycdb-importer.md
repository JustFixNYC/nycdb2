...
One-time migration aid: read dataset definitions from [nycdb/nycdb](https://github.com/nycdb/nycdb) and emit **draft** `datasets/*.yml` stubs for review in this definition repo.
...
- Python 3.12+ with `opendata-etl` dev dependencies installed and importable.
...
2. **`name`** — `derive_column_name(header)` from the framework column naming helper.
...
From this repo, with `opendata-etl` installed in the environment:
...
python3 scripts/import_legacy_nycdb.py --parity-step15 --dry-run
```
...
```bash
python3 /path/to/opendata-etl/scripts/validate_definitions.py --repo import_drafts
